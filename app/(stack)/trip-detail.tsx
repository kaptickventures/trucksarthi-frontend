...
<body>
  ...
  const buildBiltyHtml = (doc: any) => {
    return buildBiltyPreviewHtml(doc, {
      company: {
        name: companyProfile?.name || user?.company_name || user?.name || "TRUCKSARTHI",
        gstin: companyProfile?.gstin || user?.gstin || (user as any)?.kyc_data?.gstin_details?.gstin || "-",
        address: companyProfile?.address || user?.address || (user as any)?.kyc_data?.gstin_details?.principal_place_address || "-",
        phone: companyProfile?.phone ? formatPhoneNumber(companyProfile.phone) : user?.phone ? formatPhoneNumber(user.phone) : "-",
        logo_url: companyProfile?.logo_url || "",
      },
      defaultTermsText: "This is electronically generated version therefore does not require signature.",
    });
  };
