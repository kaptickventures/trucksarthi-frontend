...
export default function EditTripScreen() {
  ...
  const { updateTrip, fetchTrips } = useTrips({ autoFetch: false });
  ...
  const getSelectedLabel = (type: "truck" | "driver" | "client") => {
    switch (type) {
      case "truck":
        return trucks.find((t) => String(t._id) === String(formData.truck_id))?.registration_number;
      case "driver": {
        const driver = drivers.find((d) => String(d._id) === String(formData.driver_id));
        return driver?.name || driver?.driver_name;
      }
      case "client":
        return clients.find((c) => String(c._id) === String(formData.client_id))?.client_name;
      default:
        return "";
    }
  };
  ...
  const sheetItems = useMemo<SheetItem[]>(() => {
    ...
    if (sheetType === "truck") {
      return trucks
        .map((t) => ({ label: t.registration_number, value: String(t._id) }))
        .filter((t) => t.label.toLowerCase().includes(query));
    }
    if (sheetType === "driver") {
      return drivers
        .map((d) => ({ label: d.name || d.driver_name || "Driver", value: String(d._id) }))
        .filter((d) => d.label.toLowerCase().includes(query));
    }
    if (sheetType === "client") {
      return clients
        .map((c) => ({ label: c.client_name, value: String(c._id) }))
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const getId = (obj: any) => (typeof obj === "object" ? String(obj?._id || "") : String(obj || ""));

        const startLocId = getId(trip.start_location);
        const endLocId = getId(trip.end_location);
        const startLocName = typeof trip.start_location === "object" ? String(trip.start_location?.location_name || "") : "";
        const endLocName = typeof trip.end_location === "object" ? String(trip.end_location?.location_name || "") : "";

        setFormData({
          date: trip.trip_date ? new Date(trip.trip_date) : new Date(),
          truck_id: getId(trip.truck),
          driver_id: getId(trip.driver),
          client_id: getId(trip.client),
          ...
          advance: String((trip as any).advance ?? ""),
          notes: String(trip.notes || ""),
        });

        setStartQuery(startLocName);
        setEndQuery(endLocName);
      } catch {
  ...
  }, [tripId, fetchClients, fetchDrivers, fetchTrucks, fetchLocations, router]);

  useEffect(() => {
    // Fill location labels after locations load if query strings are still blank.
    if (!locations.length) return;
    if (!startQuery && formData.start_location_id) {
      const found = locations.find((l) => String(l._id) === String(formData.start_location_id));
      if (found?.location_name) setStartQuery(found.location_name);
    }
    if (!endQuery && formData.end_location_id) {
      const found = locations.find((l) => String(l._id) === String(formData.end_location_id));
      if (found?.location_name) setEndQuery(found.location_name);
    }
  }, [locations, formData.start_location_id, formData.end_location_id, startQuery, endQuery]);
  ...
  const handleSave = async () => {
    ...
    if (!tripId || !truck_id || !driver_id || !client_id || !trimmedStart || !trimmedEnd || !cost_of_trip) {
    ...
    try {
      ...
      if (!endLocationId) {
        ...
        if (existingEnd) {
          endLocationId = String(existingEnd._id);
        } else {
          const created = await addLocation({ location_name: trimmedEnd, complete_address: trimmedEnd });
          endLocationId = String(created._id);
        }
      }

      await updateTrip(String(tripId), {
        truck: truck_id,
        driver: driver_id,
        client: client_id,
        ...
        trip_date: formData.date.toISOString(),
      });
    ...
    <SafeAreaView edges={["left", "right", "bottom"]} style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      >
        <View className="mb-3 flex-row items-center justify-between">
          <View>
            <Text className="text-[24px] font-black" style={{ color: colors.foreground }}>{t("editTrip")}</Text>
                ...
                {startSuggestions.map((loc) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      setFormData((prev) => ({ ...prev, start_location_id: String(loc._id) }));
                    }}
                  >
                ...
                {endSuggestions.map((loc) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      setFormData((prev) => ({ ...prev, end_location_id: String(loc._id) }));
                    }}
                  >
                    <Text style={{ color: colors.foreground }} numberOfLines={1}>{loc.location_name}</Text>
                  </TouchableOpacity>
        ...
        <BottomSheetView style={{ padding: 16, gap: 12 }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
            <Text style={{ fontSize: 18, fontWeight: "800", color: colors.foreground }}>
              {sheetType === "truck" ? "Select Truck" : sheetType === "driver" ? "Select Driver" : "Select Client"}
            </Text>
      ...
      <ClientFormModal
        ...
        onSubmit={async () => {
          ...
          setFormData((p) => ({ ...p, client_id: String(res._id) }));
      ...
      <DriverFormModal
        ...
        onSubmit={async () => {
          ...
          try {
            ...
            setFormData((p) => ({ ...p, driver_id: String(res._id) }));
      ...
      <TruckFormModal
        ...
        onSubmit={async () => {
          ...
          setFormData((p) => ({ ...p, truck_id: String(res._id) }));
