...
export default function TruckManager() {
  ...
  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity
      activeOpacity={0.7}
      onPress={() => router.push({ pathname: "/(stack)/truck-manager-options", params: { truckId: item._id } } as any)}
      style={{
          ...
          ListHeaderComponent={
            <View style={{ paddingBottom: 16 }}>
              <View className="mb-3 px-0">
                <Text className="text-[24px] font-black" style={{ color: colors.foreground }}>Truck Manager</Text>
                <Text className="text-sm opacity-60" style={{ color: colors.foreground }}>Manage your fleet trucks</Text>
              </View>
