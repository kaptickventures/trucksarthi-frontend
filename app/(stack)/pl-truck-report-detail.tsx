...
export default function PLTruckReportDetailScreen() {
    ...
    }, [entityId, fetchTrucks, fetchTransactions]);
    ...
    const filtered = useMemo(() => {
        ...
        const base = (transactions || [])
            .filter((tx: any) => toRefId(tx.truckId) === entityId)
        ...
        const withBalance = base.map((tx: any) => {
            ...
            if (isIncome) currentBalance += Number(tx.amount || 0);
            else currentBalance -= Number(tx.amount || 0);
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            <tr>
              ...
              <td class="${direction === "INCOME" ? "pos" : "neg"}">${escapeHtml(signedAmount)}<br/><span style="font-size: 8px; color: #6b7280;">Bal: Rs ${balanceAt.toLocaleString()}</span></td>
...
<body>
  <h1>Truck P&L Detail Report</h1>
  <div class="sub">Truck: ${escapeHtml(entityName)}</div>
  <div class="meta">
  ...
  <table>
    ...
    <tbody>
      ${rowsHtml || `<tr><td colspan="8">No transactions found.</td></tr>`}
    </tbody>
  </table>
                ...
                <View style={{ backgroundColor: colors.card, borderRadius: 16, borderWidth: 1, borderColor: colors.border, padding: 14 }}>
                    ...
                    {!loading && filtered.length === 0 ? (
                        <Text style={{ color: colors.mutedForeground, textAlign: "center", marginTop: 8 }}>
                            No transactions found for this truck.
                        </Text>
                    ) : null}
                </View>
            </ScrollView>
